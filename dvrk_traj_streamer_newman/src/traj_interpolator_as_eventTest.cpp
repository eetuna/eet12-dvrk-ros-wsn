...
void TrajActionServer::check_robot_state()
{
    cout << "in check_robot_state function" << endl;
    cout << "robot_state: " << robot_state << endl;
    cout << "robot_state_pre: " << robot_state_pre << endl;
...
bool TrajActionServer::isDataLoaded()
{
    return m_bNewState;
}
...
void TrajActionServer::state_joint_current_callback(const sensor_msgs::JointState& jointState)
{
  //ROS_INFO(getCallerID() + " -> current state is %s", data.data);
    //Eigen::VectorXd position_joint_desired;
        position_joint_current.resize(7);

        for (int i=0;i<7;i++)
        {
            position_joint_current[i] = jointState.position[i];
        }
        /*
...
Eigen::VectorXd TrajActionServer::get_joint_current()
{
...
Eigen::VectorXd TrajActionServer::get_joint_error(Eigen::VectorXd qvec_desired)
{
  //ROS_INFO(getCallerID() + " -> current state is %s", data.data);
    //Eigen::VectorXd position_joint_desired;
        ...
        position_joint_error.resize(7);

        for (int i=0;i<7;i++)
        {
            position_joint_error[i] = position_joint_current[i] - qvec_desired[i];
        }
        /*
        position_joint_desired[0] = jointState.position[0];
        position_joint_desired[1] = jointState.position[1];
...
void TrajActionServer::executeCB(const actionlib::SimpleActionServer<davinci_traj_streamer::trajAction>::GoalConstPtr& goal) {
    ...
    Eigen::VectorXd position_joint_error, position_joint_current_;
    while (working_on_trajectory) {
        ...
        myfile << "Desired joint position: [" << qvec_new[0] << ", " << qvec_new[1] << ", " << qvec_new[2] << ", " << qvec_new[3] << ", " << qvec_new[4] << ", " << qvec_new[5] << ", " << qvec_new[6] << "]"<< endl;
        myfile << "Actual joint position: [" << position_joint_current_[0] << ", " << position_joint_current_[1] << ", " 
        << position_joint_current_[2] << ", " << position_joint_current_[3] << ", " << position_joint_current_[4] << ", " 
        << position_joint_current_[5] << ", " << position_joint_current_[6] << "]"<< endl;
        myfile << "Error joint position: [" << position_joint_error[0] << ", " << position_joint_error[1] << ", " << position_joint_error[2] << ", " << position_joint_error[3] 
